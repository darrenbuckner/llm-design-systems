...
Brutalist minimalism - pure black (#000000) background, pure white (#FFFFFF) text, maximum contrast. No grays, no accent colors, no gradients.
...
- Borders/dividers: White 1px lines
...
- Thin white horizontal rules (1px) as section dividers
...
- Outline style only - thin white border (1-2px)
- Rounded corners (~24px radius)
- Text-only interior, no fill
- No shadows, completely flat
- Generous padding (12-16px vertical, 24-32px horizontal)
- Hover: subtle inverse or slight border weight increase

### Cards/Tiles

- Black background (same as page) or thin white border (1px)
...
- Horizontal divider (1px white) to separate from content
- Left-aligned logo/branding, right-aligned actions/links
- Generous padding (20-24px vertical)

### Sections

- Full-width thin white dividers (1px) between major sections
...
- Thin white borders (1px) for table cells
- Generous cell padding (16-20px)
- No alternating row colors - use spacing for clarity

### Forms

- Input fields: white border (1px), no fill or transparent
...
1. **Tech-forward** - Monospace touches, lowercase conventions signal builder/maker culture
...
Confident, direct, no decoration needed. Builder/maker culture. This design says: “We’re focused on substance. If you need flashy, this isn’t for you.”
...
```
Create a [page type] using Block's brutalist aesthetic:
...
- Thin white borders/dividers (1px)
...
```
Design a [presentation type] with Block's brutalist aesthetic:

- Black slides (#000000), white text (#FFFFFF) throughout
- Minimal sans-serif typography
- Monospace lowercase section headers
- One key message per slide
- Thin line dividers only (1px white)
...
```
Format this [document type] in Block's brutalist style:
...
```
Create a [page/interface] using Block's brutalist aesthetic with [2/3/4]-column grid:
...
- ☐ Is it pure black/white (no grays or colors)?
...
- ☐ Are dividers thin (1px) white lines?
...
- Use `background: #000; color: #fff;`
- Border: `border: 1px solid #fff;`
- Grid: CSS Grid or Flexbox with explicit gap values
- Buttons: `border: 1px solid #fff; border-radius: 24px; background: transparent;`
